...
export function SpectralPlot({ openLoop, title }: { openLoop: Eigen[]; title: string }) {
    ...
    const xVals = openLoop.map((v) => v.real);
    const yVals = openLoop.map((v) => v.imag);
                ...
                <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} className="bg-black/30 border border-white/10 rounded">
                    ...
                    {openLoop.map((p, i) => (
                        <circle
                            ...
                            cy={yScale(p.imag)}
                            r={4}
                            fill={p.real > 0 ? "#f59e0b" : "#34d399"}
                        />
                    ))}
                </svg>
            </div>
            <div className="mt-2 text-[10px] font-mono text-white/40">
                Green: stable mode (Re &lt; 0), Amber: unstable mode (Re &gt; 0)
            </div>
        </div>
    );
}
